...
TPLazySearch::TPLazySearch(const Options &opts)
    : TPSearchEngine(opts),
...
void TPLazySearch::set_pref_operator_heuristics(
    vector<Heuristic *> &heur) {
    preferred_operator_heuristics = heur;
}

void TPLazySearch::initialize() {
    cout << "Conducting TP lazy best first search, (real) bound = " << bound << endl;
...
void TPLazySearch::get_successor_operators(vector<const GlobalOperator *> &ops) {
    assert(ops.empty());
	...
	//		std::cout << op->get_name() << " " << op->is_marked() << "\n";
	//}
...
void TPLazySearch::generate_successors() {
	...
	if ( TP_DEBUG ) {
		...
		tp_search_space.trace_path(current_state, path);
		for (const GlobalOperator *op : path) {
			std::cout << " (" << op->get_name() << ")";
		}
		std::cout << "\n";
	}
	...
	TPSearchNode current_node = tp_search_space.get_node( current_state );
    for (const GlobalOperator *op : operators)
		if (tp_search_space.is_applicable(current_state, op)) {
			if ( TP_DEBUG )
				std::cout << op->get_name() << " applicable!\n";
		...
		else if (op->is_marked()) op->unmark();
}

SearchStatus TPLazySearch::fetch_next_state() {
    if (open_list->empty()) {
    ...
    TPSearchNode pred_node = tp_search_space.get_node(current_predecessor);
...
SearchStatus TPLazySearch::step() {
    // Invariants:
    ...
    TPSearchNode node = tp_search_space.get_node(current_state);
    ...
    if (node.is_new() || reopen) {
        ...
        TPSearchNode parent_node = tp_search_space.get_node(parent_state);
...
void TPLazySearch::reward_progress() {
    open_list->boost_preferred();
}

void TPLazySearch::print_checkpoint_line(int g) const {
    cout << "[g=" << g << ", ";
    statistics.print_basic_statistics();
    cout << "]" << endl;
}

void TPLazySearch::print_statistics() const {
    statistics.print_detailed_statistics();
    tp_search_space.print_statistics();
}
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    TPLazySearch *engine = nullptr;
    if (!parser.dry_run()) {
        engine = new TPLazySearch(opts);
        /*
...
static Plugin<SearchEngine> _plugin("tp_lazy", _parse);
static Plugin<SearchEngine> _plugin_greedy("tp_lazy_greedy", _parse_greedy);
static Plugin<SearchEngine> _plugin_weighted_astar("tp_lazy_wastar", _parse_weighted_astar);
}

...
TPIteratedSearch::TPIteratedSearch(const Options &opts)
    : TPSearchEngine(opts),
...
void TPIteratedSearch::initialize() {
}

SearchEngine *TPIteratedSearch::get_search_engine(
...
SearchEngine *TPIteratedSearch::create_phase(int p) {
...
SearchStatus TPIteratedSearch::step() {
    ...
    if (last_phase_found_solution) {
        ...
        if (plan_cost < best_bound) {
			TPSearchEngine *tp_search = dynamic_cast<TPSearchEngine *>(current_search);
			if (tp_search)
				save_temporal_plan(tp_search->get_temporal_plan(), true);
...
SearchStatus TPIteratedSearch::step_return_value() {
...
void TPIteratedSearch::print_statistics() const {
    cout << "Cumulative statistics:" << endl;
    statistics.print_detailed_statistics();
}

void TPIteratedSearch::save_plan_if_necessary() const {
    // We don't need to save here, as we automatically save after
    // each successful search iteration.
}

static SearchEngine *_parse(OptionParser &parser) {
    parser.document_synopsis("TP iterated search", "");
    ...
    if (parser.help_mode()) {
        ...
        return new TPIteratedSearch(opts);
    }
}

static Plugin<SearchEngine> _plugin("tp_iterated", _parse);
}

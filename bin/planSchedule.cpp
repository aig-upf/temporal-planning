...
std::string atom2string(Domain & d, Ground* g, StringVec gaction){
  ...
  for ( unsigned i = 0; i < g->params.size(); ++i )
	...
	ss <<" "<< gaction[g->params[i]+1];
      ...
      ss <<" "<< d.types[g->lifted->params[i]]->object( g->params[i] ).first;
    }
  ss << ")";  
  return ss.str();
}
...
public:
  ...
  void readClassicalPlan(const std::string & planFileName,  Domain * t_d,  Domain * c_d){  
	...
	std::stringstream ss;
	while ( f.s[c] != ' ' && f.s[c] != '\t' && f.s[c] != '\r' && f.s[c] != '\n' && f.s[c] != '\f' && f.s[c] != '(' && f.s[c] != ')'){
	  ss << ( 97 <= f.s[c] && f.s[c] <= 122 ? (char)( f.s[c++] - 32 ) : f.s[c++] );
	}
  ...
  void compute_causal_links(Domain * c_d){    
    for (unsigned i = 0; i < _actions.size(); ++i){
      for (unsigned m = 0; m < c_d->actions[_cindexes[i]]->addEffects().size(); ++m) {	
	Ground * gi = dynamic_cast< Ground * >(c_d->actions[_cindexes[i]]->addEffects()[m]);
	if ((!TP_SHE&&gi->name=="EMPTYSTACK") || (!TP_SHE&&gi->name=="STACK1"))
	  continue;
		 
	for (unsigned j = i+1; j < _actions.size(); ++j){
	  for (unsigned n = 0; n < c_d->actions[_cindexes[j]]->precons().size(); ++n) {
	    bool bneg=false;
	    Ground * gj = dynamic_cast< Ground * >(c_d->actions[_cindexes[j]]->precons()[n]);
	    if (gj==NULL){
	      Not * pnot = dynamic_cast< Not * >(c_d->actions[_cindexes[j]]->precons()[n]);
	      gj=pnot->cond;
	      bneg=true;
	    }

	    if (atom2string(*c_d, gi,_actions[i])== atom2string(*c_d, gj,_actions[j]) && (bneg==false)){
	      bool bthreat=false;
	      for (unsigned k = i+1; (k<j && !bthreat); ++k) {
		for (unsigned o = 0; (o < c_d->actions[_cindexes[k]]->addEffects().size() && !bthreat); ++o) {
		  Ground * gk = dynamic_cast< Ground * >(c_d->actions[_cindexes[k]]->addEffects()[o]);		  

		  if (atom2string(*c_d, gi,_actions[i])== atom2string(*c_d, gk,_actions[k]))
		    bthreat=true;
		}
	      }
	      
	      if (!bthreat){
		  _causalLinks.push_back(CausalLink(i,j, atom2string(*c_d, gi,_actions[i])));
	      }
	    }
	  }
	}
    ...
    for (unsigned i = 1; i < _actions.size(); ++i){
      for (unsigned m = 0; m < c_d->actions[_cindexes[i]]->deleteEffects().size(); ++m) {	
	Ground * gi = dynamic_cast< Ground * >(c_d->actions[_cindexes[i]]->deleteEffects()[m]);
	
	for (unsigned j = 0; j < i; ++j){
	  for (unsigned n = 0; n < c_d->actions[_cindexes[j]]->precons().size(); ++n) {
	    Ground * gj = dynamic_cast< Ground * >(c_d->actions[_cindexes[j]]->precons()[n]);	    
	    if ((gj!=NULL) && atom2string(*c_d, gi,_actions[i])== atom2string(*c_d, gj,_actions[j])){
		_causalLinks.push_back(CausalLink(j,i, atom2string(*c_d, gi,_actions[i])));
	    }
	  }
	}
      }
	...
	for (unsigned j = i+1; j < _actions.size(); ++j){
	  for (unsigned n = 0; n < c_d->actions[_cindexes[j]]->addEffects().size(); ++n) {	
	    Ground * gj = dynamic_cast< Ground * >(c_d->actions[_cindexes[j]]->addEffects()[n]);
	    
	    if (atom2string(*c_d, gi,_actions[i]) == atom2string(*c_d, gj,_actions[j])){
	      bool isprimary=false;
	      for (unsigned k = j+1; (k<_actions.size() && !isprimary); ++k) {
		for (unsigned o = 0; (o < c_d->actions[_cindexes[k]]->precons().size() && !isprimary); ++o) {
		  Ground * gk = dynamic_cast< Ground * >(c_d->actions[_cindexes[k]]->precons()[o]);		  
		  if (gk!=NULL && atom2string(*c_d, gi,_actions[i])== atom2string(*c_d, gk,_actions[k]))
		    isprimary=true;
		}
	      }	      
	      if (isprimary){
  ...
  void compute_durations(Domain * t_d, Instance * t_ins){
	...
	continue;
      ...
      if ((_actions[i][0] == "PUSH-" + t_d->actions[_tindexes[i]]->name) || (_actions[i][0] == "START-" + t_d->actions[_tindexes[i]]->name)){
	TemporalAction * t_a = (TemporalAction *) t_d->actions[_tindexes[i]];
	...
	if (remaining.size()>0){
	  remaining[remaining.size()-1]=remaining.back()-_durations[i];
	}
      }
      

      if ((_actions[i][0] == "POP-" + t_d->actions[_tindexes[i]]->name) || (_actions[i][0] == "END-" + t_d->actions[_tindexes[i]]->name)){	
  ...
  void schedule_DAG_with_critical_path(Domain * t_d, Instance * t_ins){
	...
	if (cur_vertex==_causalLinks[i]._u){
	  d[_causalLinks[i]._v] = std::max(d[_causalLinks[i]._v], d[_causalLinks[i]._u] + _durations[cur_vertex] + 0.0002);
	  indegree[_causalLinks[i]._v]--;
  ...
  friend std::ostream &operator<<( std::ostream &stream, const TemporalPlan &tp ) {
	...
	ss<< " " <<tp._actions[i][j];        
      ...
      if (SCHEDULE_DEBUG){
	stream << tp._startTimes[i] << ": (" << tp._actions[i][0] << ss.str() << ") [" << tp._durations[i] << "]" << std::endl;
      }else{
	if (tp._actions[i][0].find("DO-")==0)  // TPSHE actions
	  stream << tp._startTimes[i] << ": (" << tp._actions[i][0].substr(3) << ss.str() << ") [" <<tp._durations[i] << "]" << std::endl;
	
	if (tp._actions[i][0].find("PUSH-")==0) // TPSHE actions
	  stream << tp._startTimes[i] << ": (" << tp._actions[i][0].substr(5) << ss.str() << ") [" <<tp._durations[i] << "]" << std::endl;
	
	if (tp._actions[i][0].find("END-")==0) // TEMPO(1) actions
	  stream << tp._startTimes[i] << ": (" << tp._actions[i][0].substr(4) << ss.str() << ") [" <<tp._durations[i] << "]" << std::endl;      
      }
    }
	...
	stream << tp._actions[tp._causalLinks[i]._u] << " -> " << tp._actions[tp._causalLinks[i]._v] << ", " << tp._causalLinks[i]._atom << std::endl;
      }
    }
...
int main(int argc, char *argv[]) {
  if (argc < 4){
    std::cout << "Usage: ./planSchedule <tdomain.pddl> <cdomain.pddl> <tproblem.pddl> <cplan.txt>\n";
    exit(1);
  }

...
// valgrind --leak-check=yes bin/compileSHECosts ../domains/tempo-sat/Mapanalyser/domain/domain.pddl ../domains/tempo-sat/Mapanalyser/problems/pfile3-4-2-0-1.pddl

#include "compile.h"

int main( int argc, char *argv[] ) {
	if ( argc < 3 ) {
		std::cout << "Usage: ./compileSHECosts <domain.pddl> <task.pddl>\n";
	...
	for ( unsigned i = 0; i < d->actions.size(); ++i )
		...
		for ( IntSet::iterator j = g.depths[i].begin(); j != g.depths[i].end(); ++j )
			if ( g.outgoing( i ) ) {
				...
				if ( *j ) cd->addCost( name, 1 );
				else if ( durationIndices[i].size() ) {
